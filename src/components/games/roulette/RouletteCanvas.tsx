import type { RouletteSlice } from './Roulette.types'

export const ROULETTE_SZ = 340
const CX = ROULETTE_SZ / 2
const CY = ROULETTE_SZ / 2
const R = 158
const PIN_R = R + 2
const PIN_SIZE = 6

function wrapText(ctx: CanvasRenderingContext2D, str: string, maxW: number): string[] {
  const words = str.split(' ')
  const lines: string[] = []
  let line = ''
  ctx.font = 'bold 9.5px sans-serif'
  for (const w of words) {
    const test = line ? line + ' ' + w : w
    if (ctx.measureText(test).width > maxW && line) {
      lines.push(line)
      line = w
    } else {
      line = test
    }
  }
  if (line) lines.push(line)
  return lines
}

export function drawWheel(
  ctx: CanvasRenderingContext2D, 
  angle: number, 
  cupcake: HTMLImageElement | null,
  slices: RouletteSlice[]
) {
  ctx.clearRect(0, 0, ROULETTE_SZ, ROULETTE_SZ)
  const n = slices.length
  const arc = (2 * Math.PI) / n

  // 1. Aro exterior café
  ctx.save()
  ctx.beginPath()
  ctx.arc(CX, CY, R + 10, 0, 2 * Math.PI)
  ctx.fillStyle = '#5a2e00'
  ctx.fill()
  ctx.restore()

  // 2. Segmentos alternados
  for (let i = 0; i < n; i++) {
    const s  = slices[i]
    const sa = angle + i * arc
    const ea = sa + arc

    ctx.save()
    ctx.beginPath()
    ctx.moveTo(CX, CY)
    ctx.arc(CX, CY, R, sa, ea)
    ctx.closePath()
    ctx.fillStyle = s.color
    ctx.fill()
    ctx.strokeStyle = '#5a2e00'
    ctx.lineWidth = 2
    ctx.stroke()
    ctx.restore()

    // Texto y emoji radial
    const mid = sa + arc / 2
    ctx.save()
    ctx.translate(CX, CY)
    ctx.rotate(mid)

    // Emoji
    ctx.font = '13px sans-serif'
    ctx.textAlign = 'center'
    ctx.textBaseline = 'middle'
    ctx.fillText(s.emoji, 28, 0)

    // Texto multilinea
    const textR = R * 0.68
    const lines = wrapText(ctx, s.text, 78)
    const lh    = 12
    const tot   = lines.length * lh
    ctx.font = 'bold 9.5px sans-serif'
    ctx.fillStyle = s.tcolor
    ctx.textAlign = 'center'
    ctx.textBaseline = 'middle'
    lines.forEach((l, li) => {
      ctx.fillText(l, textR, -tot / 2 + li * lh + lh / 2)
    })

    ctx.restore()
  }

  // 3. Topes metálicos (pins)
  for (let i = 0; i < n; i++) {
    const pinAngle = angle + i * arc
    const px = CX + PIN_R * Math.cos(pinAngle)
    const py = CY + PIN_R * Math.sin(pinAngle)

    ctx.save()
    ctx.beginPath()
    ctx.arc(px + 1, py + 1, PIN_SIZE, 0, 2 * Math.PI)
    ctx.fillStyle = 'rgba(0,0,0,0.25)'
    ctx.fill()
    ctx.restore()

    ctx.save()
    ctx.beginPath()
    ctx.arc(px, py, PIN_SIZE, 0, 2 * Math.PI)
    ctx.fillStyle = '#e8e0d0'
    ctx.fill()
    ctx.strokeStyle = '#8a6a40'
    ctx.lineWidth = 1.2
    ctx.stroke()
    ctx.beginPath()
    ctx.arc(px - 1.5, py - 1.5, PIN_SIZE * 0.38, 0, 2 * Math.PI)
    ctx.fillStyle = 'rgba(255,255,255,0.7)'
    ctx.fill()
    ctx.restore()
  }

  // 4. Centro con cupcake
  ctx.save()
  const cR = 46
  ctx.beginPath()
  ctx.arc(CX, CY, cR, 0, 2 * Math.PI)
  ctx.fillStyle = '#fff'
  ctx.fill()
  ctx.strokeStyle = '#5a2e00'
  ctx.lineWidth = 3.5
  ctx.stroke()

  if (cupcake && cupcake.complete && cupcake.naturalWidth) {
    const s = cR * 1.72
    ctx.drawImage(cupcake, CX - s / 2, CY - s / 2, s, s)
  } else {
    ctx.font = '36px sans-serif'
    ctx.textAlign = 'center'
    ctx.textBaseline = 'middle'
    ctx.fillText('🧁', CX, CY + 2)
  }
  ctx.restore()
}
